...
def get_wd_mids_set(fbsubset):
    print('get all mids in the Freebase subset...')
    lines_seen = set()  # holds lines already seen
    # cleanfile = "processed/cleanedWD.txt"
    ...
    for i, line in enumerate(open(fbsubset, "r")):
        ...
        entity1 = items[0]
        line = "{}\t{}\t{}\n".format(entity1, items[2], items[1])
...
def findsetgrams(dataset):
    ...
    for fname in ["annotated_wd_data_valid_answerable", "annotated_wd_data_test_answerable"]:
...
if __name__ == '__main__':
    ...
    cleanfile = os.path.join(args.output, 'cleanedWD.txt')
    # get_wd_mids accepts argument processed/cleanedWD.txt (basically the subgraph)
    fb_mids = get_wd_mids_set(args.wdsubset)        
